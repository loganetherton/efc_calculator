 ...
 * FORM VARIABLES
 * 
 * Line 1: adjusted_gross_income // GOOD
 * Line 2A: student_income // GOOD
 * Line 2B: spouse_income // GOOD
 * Line 2: total_income_from_work // GOOD
 * Line 3: taxable_income // GOOD
 * Line 4: untaxed_income_benefits // GOOD
 * Line 5: taxable_untaxable_income // GOOD
 * Line 6: total_additional_financial_information // GOOD
 * Line 7: total_income // GOOD
 * Line 8: income_tax_paid // GOOD
 * Line 9: state_tax_allowance // GOOD
...
$(document).on('change', "#income_tax_paid", function(){
    ...
    // Line 18: Direct from user input
});
    
/**
 * LINE 19
 */
$(document).on('change', "#net_worth_investments", function(){
    cantBeNegative(this);
});
 ...
 * LINE 20
 */
$(document).on('change', "#net_worth_business_farm", function(){
    cantBeNegative(this);
    // Line 21: Table B3 and line 20
    determineAdjustedNetWorthBusinessFarm();
    // Line 22: Sum line 18, 19, and 21
    data('net_worth_assets',
         dataObj.cash_savings_checking +
         dataObj.net_worth_investments +
         dataObj.adjusted_net_worth_business_farm);
    // Line 23: Table b4, age of student, and whether married
    student_age = calcAge('student');
    data('student_age',student_age);
    determineAssetProtectionAllowance();
    // Line 24: Line 22 - line 23
    data('discretionary_net_worth', dataObj.net_worth_assets - dataObj.asset_protection_allowance);
    // Line 25: CONSTANT 0.2
    // Line 26: Line 24 * 0.2
    student_contribution_from_assets = dataObj.discretionary_net_worth * 0.2;
    if(student_contribution_from_assets < 0){
        student_contribution_from_assets = 0;
    }
    data('student_contribution_from_assets', student_contribution_from_assets);
    // Line 27: Line 17 + line 26
    data('contribution_from_ai_and_assets', dataObj.student_contribution_from_ai + dataObj.student_contribution_from_assets);
    ...
    // Set line 22 to 0
    data('discretionary_net_worth_efc_lowered', 0);
    // Use line 23 (asset_protection_allowance)
    // Line 24 lowered (discretionary_net_worth_efc_lowered)
    discretionary_net_worth_efc_lowered = dataObj.discretionary_net_worth_efc_lowered - dataObj.asset_protection_allowance;
    data('discretionary_net_worth_efc_lowered',discretionary_net_worth_efc_lowered);
    // Line 26 (student_contribution_from_assets_efc_lowered)
    contribution_from_assets_lowered_efc = dataObj.discretionary_net_worth_efc_lowered * 0.2;
    if(contribution_from_assets_lowered_efc < 0){
        contribution_from_assets_lowered_efc = 0;
    }
    data('contribution_from_assets_lowered_efc', contribution_from_assets_lowered_efc);
    // Line 27 (contribution_from_ai_and_assets_lowered_efc)
    contribution_from_ai_and_assets_lowered_efc = dataObj.student_contribution_from_ai + dataObj.contribution_from_assets_lowered_efc;
    data('contribution_from_ai_and_assets_lowered_efc',contribution_from_ai_and_assets_lowered_efc);
    // Line 29: contribution_from_ai_and_assets_lowered_efc / number in college
    lowered_efc_nine_months = dataObj.contribution_from_ai_and_assets_lowered_efc / dataObj.number_in_college;
    if(lowered_efc_nine_months < 0){
        lowered_efc_nine_months = 0;
    }
...
$(document).on('change', '#months_enrolled', function(){
    ...
    if(dataObj.months_enrolled < 9){
        ...
        // Use lowered_efc_nine_months
        efc_less_than_nine_months_lowered_per_month = dataObj.lowered_efc_nine_months / 9;
        data('efc_less_than_nine_months_lowered_per_month',efc_less_than_nine_months_lowered_per_month);
        // Multiple by number of months attending
        efc_less_than_nine_months_lowered = dataObj.efc_less_than_nine_months_lowered_per_month * dataObj.months_enrolled;
        data('efc_less_than_nine_months_lowered',efc_less_than_nine_months_lowered);
